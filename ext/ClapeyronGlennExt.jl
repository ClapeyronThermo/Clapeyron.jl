...
function Clapeyron.__glenn_jl(calc, input; Rgas=Clapeyron.Rgas(), R0=calc.R_ref, reference_state=nothing, verbose=false, strict=true)
...
function __thermocalcerror(T, name, id)
    throw(Glenn.ThermoCalcError("Temperature $T K is out of valid range for '$(name)'. " * "Use get_species_data(model, $id) to check available intervals."))
end
