function ForwardDiff.DerivativeConfig(f::F, y::AbstractArray{Y}, x::X, tag::T=maketag(f, X)) where {F<:WithContext,X<:Real,Y<:Real,T}
    duals = similar(y, Dual{T,Y,1})
    return ForwardDiff.DerivativeConfig{T,typeof(duals)}(duals)
end

function ForwardDiff.GradientConfig(f::F, x::AbstractArray{V}, chunk::Chunk{N}=Chunk(x), tag::T=maketag(f, V)) where {F<:WithContext,V,N,T}
...
function JacobianConfig(f::F, x::AbstractArray{V}, chunk::Chunk{N}=Chunk(x), tag::T=maketag(f, V)) where {F<:WithContext,V,N,T}
    seeds = ForwardDiff.construct_seeds(Partials{N,V})
    duals = similar(x, Dual{T,V,N})
    return ForwardDiff.JacobianConfig{T,V,N,typeof(duals)}(seeds, duals)
end

function ForwardDiff.JacobianConfig(f::F, y::AbstractArray{Y}, x::AbstractArray{X}, chunk::Chunk{N}=Chunk(x), tag::T=maketag(f, X)) where {F<:WithContext,Y,X,N,T}
...
function ForwardDiff.HessianConfig(f::F, x::AbstractArray{V}, chunk::Chunk=Chunk(x), tag=maketag(f, V)) where {F<:WithContext,V}
...
function ForwardDiff.HessianConfig(f::F, result::DiffResults.DiffResult, x::AbstractArray{V}, chunk::Chunk=Chunk(x), tag=maketag(f, V)) where {F<:WithContext,V}
