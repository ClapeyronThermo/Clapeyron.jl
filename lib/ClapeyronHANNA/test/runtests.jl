...
@testset "HANNA" begin
    systems = Dict(["water", "ethanol", "methanol"]        => ([0.277717203, 0.278312653, -0.033341952], ["O", "CCO", "CO"]), ["dmso", "ethanol", "aspirin"]          => ([-0.047125854, -0.071092166, 0.233126670], ["CS(=O)C", "CCO", "CC(=O)Oc1ccccc1C(=O)O"]), ["saccharin", "methanol", "chloroform"] => ([0.028507818, 0.408137769, 0.330980510], ["C1=CC=C2C(=C1)C(=O)NS2(=O)=O", "CO", "ClC(Cl)Cl"]))
    ...
    for (system_i, (lnγs_ref_i, smiles_i)) in systems
        ...
        γs_i = activity_coefficient(model, 1e5, 300.0, [0.5, 0.3, 0.2])
        ...
        γs_i_smiles = activity_coefficient(model_smiles, 1e5, 300.0, [0.5, 0.3, 0.2])
