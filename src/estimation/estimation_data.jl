...
function EstimationData(table_data, method, loss; output_weights=nothing, data_weights=nothing, species=nothing, normalize=true, source=nothing)
    ...
    return EstimationData{typeof(method),typeof(loss),N,M}(method, loss, _species, inputs_name, outputs_name, inputs, outputs, valid, inputs_ismissingvalues, outputs_ismissingvalues, inputs_error, outputs_error, inputs_errortype, outputs_errortype, out_w, data_w, normalize, _source)
end
    ...
    for (j, var_name) in enumerate(pure_names)
        for etype in ERRORTYPES
            ...
            if err_header in colnames
                ...
                for i in 1:nrows
                    tup = error_vec[i]
                    error_vec[i] = NTuple{len_tuple,Float64}(k == j ? err_col[i] : tup[k] for k in 1:len_tuple)
                end
