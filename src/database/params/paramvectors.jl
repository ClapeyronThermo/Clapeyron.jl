...
"""
    Compressed3DMatrix{T,V<:AbstractVector{T}}

struct used internally to store structure data.

"""
struct Compressed3DMatrix{T,V<:AbstractVector{T}} 
    values::V
    outer_indices::Vector{Tuple{Int}} #index of components
    inner_indices::Vector{Tuple{Int,Int}} #index of groups
    outer_size::Tuple{Int} #size of component matrix
    inner_size::Tuple{Int,Int} #size of groups matrices
end

function Base.show(io::IO,mime::MIME"text/plain",m::Compressed3DMatrix{T}) where T
    n = length(m.values)
    println(io,typeof(m)," with ",n," entr",(n == 1 ? "y:" : "ies:"))
    for (idx,i,(k,l)) in indices(m)
        if idx != 1
        println(io)
        end
        print(io," ",(i)," >=< ",(a,b),": ",m.values[idx])
    end
end

function Base.show(io::IO,m::Compressed3DMatrix{T}) where T
    print(io,typeof(m))
    print(io,m.values)
end
function Compressed3DMatrix{T}() where T
    return Compressed3DMatrix(T[],Tuple{Int}[],Tuple{Int,Int}[],(0,0),(0,0))
end

function Compressed3DMatrix(x::VectorofMatrices{T}) where T
    outer_size = size(x)
    is1, is2 = 0
    os1 = outer_size
    for i in eachindex(x)
    ...
    if iszero(os1)
        return Compressed3DMatrix(values,outer_indices,inner_indices,outer_size,inner_size)
    end
    ...
    for i in 1:os1
        ...
        a1,a2 = size(xi)
        for a in 1:a1
            for b in 1:a2 #this includes (i,i)(a,a) (sCKSAFT)
                if !_iszero(xi[a,b]) 
                    push!(values,xi[a,b])
                    push!(outer_indices,(i))
                    push!(inner_indices,(a,b))
                end
            end
        end
    end
    return Compressed3DMatrix{T,Vector{T}}(values,outer_indices,inner_indices,outer_size,inner_size)
end

#Base.eltype(m::Compressed4DMatrix{T}) where T = T

function Base.setindex!(m::Compressed3DMatrix,val,i::Int)
    @inbounds begin
        m.values[i] = val
    end
end
...
=#
function indices(x::Compressed3DMatrix)
    xin = x.outer_indices
    l = 1:length(xin) 
    return zip(l,xin,x.inner_indices)
end
...
# Operations
function Base.:(+)(matrix::Compressed3DMatrix, x::Number)
    values = matrix.values .+ x
