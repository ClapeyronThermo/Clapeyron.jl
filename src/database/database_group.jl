...
function _parse_group_string(gc::String, gctype=String)
    ...
    if startswith(gc_strip, "[") && endswith(gc_strip, "]")
        ...
        for (i, gcpair) ∈ pairs(gcpairs)
            ...
            if (startswith(raw_group_i, "\"") && endswith(raw_group_i, "\"")) || (startswith(raw_group_i, "(") && endswith(raw_group_i, ")"))
...
function GroupParam(gccomponents::Union{AbstractVector,Tuple{String,Vector},Pair{String,Vector}}, group_locations=String[]; group_userlocations=String[], verbose::Bool=false, grouptype=:unknown)
...
function GroupParam(gccomponents, grouplocations=String[], options::ParamOptions=DefaultGroupOptions, grouptype=:unknown)
