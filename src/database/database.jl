...
if CSV_V1
    function read_csv(filepath,options::ParamOptions,sep = :auto)::CSV.File
        ...
        dropcols = String[]
        cols = getline(filepath,3)
        

        for colname in eachsplit(cols,_delim)
            norm_colname = normalisestring(colname; tofilter=r"[ \-\_\d]")
            if norm_colname in ignorelist
                push!(dropcols,colname)
            end
        end
        if is_inline_csv(filepath)
            df = CSV.File(IOBuffer(filepath); header=3,normalizenames=true, on_error=:collect,drop = dropcols, delim = _delim,buffer_in_memory = true)
        else
            df = CSV.File(filepath; header=3,on_error=:collect, drop = dropcols,delim = _delim)
        end
        return df
    end
else
