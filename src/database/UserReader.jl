...
```
"""
function ParamTable(type, data; location::Union{String,Nothing}=nothing, name::Union{String,Nothing}=nothing, grouptype=:unknown, options::ParamOptions=DefaultOptions)
    type = Symbol(type)
    ...
    io = open(file, "w")
    pretext = """Clapeyron Database File
