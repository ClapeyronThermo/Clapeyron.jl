...
function convergence(xold, xi, atol=zero(eltype(xi)), rtol=8eps(one(eltype(xi))), lognorm=false, normorder=Inf)
...
function fixpoint(f, x0, method::AbstractFixPoint=SSFixPoint(); atol=zero(eltype(x0)), rtol=8eps(one(eltype(x0))), max_iters=100, return_last=false)
...
function _fixpoint(f::F, x0::T, method::SSFixPoint, atol::T=zero(T), rtol::T=8*eps(T), max_iters=100, return_last=false) where {F,T<:Real}
    nan = (0*atol)/(0*atol)
    xi = f(x0)
...
function _fixpoint(f::F, x00::T, method::AitkenFixPoint, atol::T=zero(T), rtol::T=8*eps(T), max_iters=100, return_last=false) where {F,T<:Real}
    nan = (0*atol)/(0*atol)
    itercount = 2
    x3 = x00
    x1 = f(x3)
...
function _fixpoint(f!::F, x0::X where {X<:AbstractVector{T}}, method::SSFixPoint, atol::T=zero(T), rtol::T=8*eps(T), max_iters=100, return_last=false) where {F,T<:Real}
    nan = (0*atol)/(0*atol)
    xi = copy(x0)
    xi = f!(xi, x0)
    α = method.dampingfactor
