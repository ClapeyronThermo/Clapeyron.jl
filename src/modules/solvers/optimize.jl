...
function only_fg!(fg!::T) where T
    ...
    return ScalarObjective(f=f, g=g, fg=fg, fgh=nothing, h=nothing)
end
...
function _1var_optimize_quad(f, x0, options)
    ...
    return NLSolvers.ConvergenceInfo(BoundOptim1Var(), (; solution=xx, f0=f3, minimum=fx, time=time() - time0, iter=iter_x), options)
end
