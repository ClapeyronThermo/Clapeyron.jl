...
function get_json_data(components; userlocations=String[], coolprop_userlocations=true, verbose=false)
...
function SingleFluid(components; userlocations=String[], ancillaries=nothing, ancillaries_userlocations=String[], estimate_pure=false, coolprop_userlocations=true, Rgas=nothing, verbose=false, idealmodel=nothing, ideal_userlocations=String[], allow_pseudo_pure=false)
...
```
"""
function SingleFluidIdeal(components; userlocations=String[], Rgas=nothing, verbose=false, coolprop_userlocations=true, idealmodel=nothing, ideal_userlocations=String[])
    _components = format_components(components)
...
function __verbose_found_json_terms(data)
    ...
    for data_i in data
        type = data_i[:type]
        additional = if type == "ResidualHelmholtzGERG2008" || type == "GERG-2008"
...
function _parse_ancilliary_func(anc, input_key, output_key)
    anc_typemap = Dict("pV" => :exp, "pL" => :exp, "rhoV" => :exp, "rhoL" => :exp, "rhoLnoexp" => :noexp, "rhoVnoexp" => :noexp, "rational" => :rational)

    anc_using_r_map = Dict("pV" => true, "pL" => true, "rhoV" => false, "rhoL" => false, "rhoLnoexp" => false, "rhoVnoexp" => false, "rational" => false)
...
function idealmodel_to_json_data(model::BasicIdealModel, Tr, T0, Vr)
    [
        Dict(:type => "IdealGasHelmholtzLead", :a1 => - log(Vr) - 1.5*log(Tr) - 1, :a2 => 0)
        Dict(:type => "IdealGasHelmholtzLogTau", :a => 1.5)
    ]
end
...
function idealmodel_to_json_data(model::ReidIdealModel, Tr, T0, Vr)
    ...
    n = length(coeffs)
    [Dict(:type => "IdealGasHelmholtzLead", :a1 => - log(Vr) - log(298) + log(Tr), :a2 => 0), Dict(:type => "IdealGasHelmholtzLogTau", :a => -1), Dict(:type => "IdealGasHelmholtzCP0PolyT", :T0 => 298.0, :Tc => Tr, :c => [coeffs...], :t => collect(0:(n - 1)))]
end
...
function idealmodel_to_json_data(model::MonomerIdealModel, Tr, T0, Vr)
    ...
    [Dict(:type => "IdealGasHelmholtzLead", :a1 => a1 - 1, :a2 => 0.0), Dict(:type => "IdealGasHelmholtzLogTau", :a => 1.5)]
end
...
function idealmodel_to_json_data(model::WalkerIdealModel, Tr, T0, Vr)
    ...
    #    for k in @groups(i)))
    [Dict(:type => "IdealGasHelmholtzLead", :a1 => a1 - 1, :a2 => 0.0), Dict(:type => "IdealGasHelmholtzLogTau", :a => 1.5 + Nroti/2), Dict(:type => "IdealHelmholtzPlanckEinsteinGeneralized", :n => n_pe, :t => t_pe, :c => c_pe, :d => d_pe), Dict(:type => "IdealGasHelmholtzPower", :n => n_power, :t => t_power)]
end
...
function idealmodel_to_json_data(model::AlyLeeIdealModel, Tr, T0, Vr)
    ...
    [Dict(:type => "IdealGasHelmholtzCP0AlyLee", :Tc => Tr, :T0 => 298.15, :c => [A, B, C, D, E]), Dict(:type => "IdealGasHelmholtzCP0AlyLee", :Tc => Tr, :T0 => 298.15, :c => [0.0, F, G, H, I]), Dict(:type => "IdealGasHelmholtzLead", :a1 => - log(Vr), :a2 => 0.0)]
end

function idealmodel_to_json_data(model::ShomateIdealModel, Tr, T0, Vr)
    coeffs = model.params.coeffs[1] ./ Rgas(model)
    n = length(coeffs)
    [Dict(:type => "IdealGasHelmholtzLead", :a1 => - log(Vr) - log(298) + log(Tr), :a2 => 0), Dict(:type => "IdealGasHelmholtzLogTau", :a => -1), Dict(:type => "IdealGasHelmholtzCP0PolyT", :T0 => 298.0, :Tc => Tr, :c => [coeffs...], :t => [0, 1, 2, 3, -2])]
end
