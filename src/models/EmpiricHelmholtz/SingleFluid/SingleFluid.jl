...
function a_ideal(model::SingleFluidIdeal, V, T, z=SA[1.0], k=__get_k_alpha0(model))
...
a_ideal(model::SingleFluid, V, T, z=SA[1.0]) = a_ideal(idealmodel(model), V, T, z)

function a_res(model::SingleFluid, V, T, z=SA[1.0])
    Tc = model.properties.Tr
    rhoc = model.properties.rhor
    N = sum(z)
    δ = N/(rhoc*V)
