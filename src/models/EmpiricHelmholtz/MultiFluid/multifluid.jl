...
`Rgas` sets the value of the gas constant to be used by the multifluid. The default is the following:
  ...
  - If `Rgas` is not specified and the input is a multi-component model, then the value of `Rgas` will be set to `Clapeyron.R̄ = Rgas() = 8.31446261815324`[J⋅K⁻¹⋅mol⁻¹] (2019 defined constant value).
"""
MultiFluid

function MultiFluid(components; idealmodel=nothing, pure_userlocations=String[], ideal_userlocations=String[], mixing=AsymmetricMixing, departure=EmpiricDeparture, mixing_userlocations=String[], departure_userlocations=String[], estimate_pure=false, estimate_mixing=:off, coolprop_userlocations=true, Rgas=nothing, reference_state=nothing, verbose=false)
    ...
    pures = [SingleFluid(comp; userlocations=pure_userlocations, idealmodel=idealmodels[i], verbose=verbose, estimate_pure=estimate_pure, coolprop_userlocations=coolprop_userlocations, Rgas=Rgas) for (i, comp) in pairs(_components)]
