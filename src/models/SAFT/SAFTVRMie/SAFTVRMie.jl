...
function B_fdf(model::SAFTVRMieModel, V, T, z, λ, x_0, ζ_X_=@f(ζ_X), ρ_S_=@f(ρ_S))
    ...
    _df = (((1-ζ_X_/2)*I/ζX3-9*ζ_X_*(1+ζ_X_)*J/(2*ζX3)) + ζ_X_*((3*(1-ζ_X_/2)*ζX2 - 0.5*ζX3)*I/ζX6 - 9*J*((1+2*ζ_X_)*ζX3 + ζ_X_*(1+ζ_X_)*3*ζX2)/(2*ζX6)))
    return _f, _df
end
...
function ∂a_2╱∂ρ_S(model::SAFTVRMieModel, V, T, z, i)
    ...
    ζ_X_ = @f(ζ_X)
    ρ_S_ = @f(ρ_S)
    ∂KHS╱∂ρ_S = -ζ_X_/ρ_S_ * ((4*(1-ζ_X_)^3*(1+4*ζ_X_+4*ζ_X_^2-4*ζ_X_^3+ζ_X_^4) + (1-ζ_X_)^4*(4+8*ζ_X_-12*ζ_X_^2+4*ζ_X_^3))/(1+4*ζ_X_+4*ζ_X_^2-4*ζ_X_^3+ζ_X_^4)^2)
    return 0.5 * @f(C, i, i)^2 * (@f(ρ_S)*∂KHS╱∂ρ_S*(x_0ij^(2*λa[i])*(@f(aS_1, 2*λa[i])+@f(B, 2*λa[i], x_0ij)) - 2*x_0ij^(λa[i]+λr[i])*(@f(aS_1, λa[i]+λr[i])+@f(B, λa[i]+λr[i], x_0ij)) + x_0ij^(2*λr[i])*(@f(aS_1, 2*λr[i])+@f(B, 2*λr[i], x_0ij))) + @f(KHS)*(x_0ij^(2*λa[i])*(@f(∂aS_1╱∂ρ_S, 2*λa[i])+@f(∂B╱∂ρ_S, 2*λa[i], x_0ij)) - 2*x_0ij^(λa[i]+λr[i])*(@f(∂aS_1╱∂ρ_S, λa[i]+λr[i])+@f(∂B╱∂ρ_S, λa[i]+λr[i], x_0ij)) + x_0ij^(2*λr[i])*(@f(∂aS_1╱∂ρ_S, 2*λr[i])+@f(∂B╱∂ρ_S, 2*λr[i], x_0ij))))
end
...
function a_dispchain(model::SAFTVRMieModel, V, T, z, _data=@f(data))
    ...
    for i ∈ comps
        ...
        a1_ij = (2*π*ϵ*dij3) * _C * ρS * (x_0ij_λa*(aS₁_a+B_a) - x_0ij_λr*(aS₁_r+B_r))
        ...
        a2_ij = π * _KHS * (1+_χ) * ρS * ϵ^2 * dij3 * _C^2 * (x_0ij_2λa*(aS₁_2a+B_2a) - 2*x_0ij_λaλr*(aS₁_ar+B_ar) + x_0ij_2λr*(aS₁_2r+B_2r))
        ...
        ∂a_1∂ρ_S = _C*(x_0ij_λa*(∂aS₁∂ρS_a+∂B∂ρS_a) - x_0ij_λr*(∂aS₁∂ρS_r+∂B∂ρS_r))
        #calculus for g1
        g_1_ = 3*∂a_1∂ρ_S - _C*(λa*x_0ij_λa*(aS₁_a + B_a) - λr*x_0ij_λr*(aS₁_r + B_r))
        θ = expm1(τ)
        γc = 10 * (-tanh(10*(0.57 - α)) + 1) * _ζst * θ * exp(_ζst*(-6.7 - 8*_ζst))
        ∂a_2∂ρ_S = 0.5 * _C^2 * (ρS_∂KHS*(x_0ij_2λa*(aS₁_2a+B_2a) - 2*x_0ij_λaλr*(aS₁_ar+B_ar) + x_0ij_2λr*(aS₁_2r+B_2r)) + _KHS*(x_0ij_2λa*(∂aS₁∂ρS_2a + ∂B∂ρS_2a) - 2*x_0ij_λaλr*(∂aS₁∂ρS_ar + ∂B∂ρS_ar) + x_0ij_2λr*(∂aS₁∂ρS_2r + ∂B∂ρS_2r)))

        gMCA2 = 3*∂a_2∂ρ_S-_KHS * _C^2 * (λr*x_0ij_2λr*(aS₁_2r+B_2r) - (λa+λr)*x_0ij_λaλr*(aS₁_ar+B_ar) + λa*x_0ij_2λa*(aS₁_2a+B_2a))
        g_2_ = (1 + γc)*gMCA2
        ...
        for j ∈ 1:(i - 1)
            ...
            dij3 = dij^3
            #calculations for a1
            a1_ij = (2*π*ϵ*dij3) * _C * ρS * (x_0ij^λa*(@f(aS_1, λa, ζₓ)+@f(B, λa, x_0ij, ζₓ)) - x_0ij^λr*(@f(aS_1, λr, ζₓ)+@f(B, λr, x_0ij, ζₓ)))

            #calculations for a2
            α = _C*(1/(λa-3)-1/(λr-3))
            f1, f2, f3, f4, f5, f6 = @f(f123456, α)
            _χ = f1*_ζst+f2*_ζst5+f3*_ζst8
            a2_ij = π * _KHS * (1+_χ) * ρS * ϵ^2 * dij3 * _C^2 * (x_0ij^(2*λa)*(@f(aS_1, 2*λa, ζₓ)+@f(B, 2*λa, x_0ij, ζₓ)) - 2*x_0ij^(λa+λr)*(@f(aS_1, λa+λr, ζₓ)+@f(B, λa+λr, x_0ij, ζₓ)) + x_0ij^(2*λr)*(@f(aS_1, 2λr, ζₓ)+@f(B, 2*λr, x_0ij, ζₓ)))
...
function a_disp(model::SAFTVRMieModel, V, T, z, _data=@f(data))
    ...
    for i ∈ comps
        ...
        a1_ij = (2*π*ϵ*dij3) * _C * ρS * (x_0ij^λa*(aS_1_a+B_a) - x_0ij^λr*(aS_1_r+B_r))
        ...
        f1, f2, f3, f4, f5, f6 = @f(f123456, α)
        _χ = f1*_ζst+f2*_ζst5+f3*_ζst8
        a2_ij = π * _KHS * (1+_χ) * ρS * ϵ^2 * dij3 * _C^2 * (x_0ij^(2*λa)*(aS_1_2a+B_2a) - 2*x_0ij^(λa+λr)*(aS_1_ar+B_ar) + x_0ij^(2*λr)*(aS_1_2r+B_2r))
        ...
        for j ∈ 1:(i - 1)
            ...
            x_0ij = σ/dij
            #calculations for a1
            a1_ij = (2*π*ϵ*dij3) * _C * ρS * (x_0ij^λa*(@f(aS_1, λa, _ζ_X)+@f(B, λa, x_0ij, _ζ_X)) - x_0ij^λr*(@f(aS_1, λr, _ζ_X)+@f(B, λr, x_0ij, _ζ_X)))

            #calculations for a2
            α = _C*(1/(λa-3)-1/(λr-3))
            f1, f2, f3, f4, f5, f6 = @f(f123456, α)
            _χ = f1*_ζst+f2*_ζst5+f3*_ζst8
            a2_ij = π * _KHS * (1+_χ) * ρS * ϵ^2 * dij3 * _C^2 * (x_0ij^(2*λa)*(@f(aS_1, 2*λa, _ζ_X)+@f(B, 2*λa, x_0ij, _ζ_X)) - 2*x_0ij^(λa+λr)*(@f(aS_1, λa+λr, _ζ_X)+@f(B, λa+λr, x_0ij, _ζ_X)) + x_0ij^(2*λr)*(@f(aS_1, 2λr, _ζ_X)+@f(B, 2*λr, x_0ij, _ζ_X)))
...
function a_chain(model::SAFTVRMieModel, V, T, z, _data=@f(data))
    ...
    for i ∈ comps
        ...
        #@show (g_HSi,i)
        ∂a_1∂ρ_S = _C*(x_0ij^λa*(∂aS_1∂ρS_a+∂B∂ρS_a) - x_0ij^λr*(∂aS_1∂ρS_r+∂B∂ρS_r))
        #@show (∂a_1∂ρ_S,1)
        ...
        ∂a_2∂ρ_S = 0.5 * _C^2 * (ρS_∂KHS*(x_0ij^(2*λa)*(aS_1_2a+B_2a) - 2*x_0ij^(λa+λr)*(aS_1_ar+B_ar) + x_0ij^(2*λr)*(aS_1_2r+B_2r)) + _KHS*(x_0ij^(2*λa)*(∂aS_1∂ρS_2a+∂B∂ρS_2a) - 2*x_0ij^(λa+λr)*(∂aS_1∂ρS_ar+∂B∂ρS_ar) + x_0ij^(2*λr)*(∂aS_1∂ρS_2r+∂B∂ρS_2r)))

        gMCA2 = 3*∂a_2∂ρ_S-_KHS * _C^2 * (λr*x_0ij^(2*λr)*(aS_1_2r+B_2r) - (λa+λr)*x_0ij^(λa+λr)*(aS_1_ar+B_ar) + λa*x_0ij^(2*λa)*(aS_1_2a+B_2a))
        g_2_ = (1+γc)*gMCA2
        #@show (g_2_,i)
...
const SAFTVRMieconsts = (
    ...
    ϕ=((7.5365557, -359.440, 1550.9, -1.199320, -1911.2800, 9236.9), (-37.604630, 1825.60, -5070.1, 9.063632, 21390.175, -129430.0), (71.745953, -3168.00, 6534.6, -17.94820, -51320.700, 357230.0), (-46.835520, 1884.20, -3288.7, 11.34027, 37064.540, -315530.0), (-2.4679820, - 0.82376, -2.7171, 20.52142, 1103.7420, 1390.2), (-0.5027200, -3.19350, 2.0883, -56.63770, -3264.6100, -4518.2), (8.0956883, 3.70900, 0.0000, 40.53683, 2556.1810, 4241.6)),
    c=[
         0.0756425183020431 -0.128667137050961 0.128350632316055 -0.0725321780970292   0.0257782547511452  -0.00601170055221687  0.000933363147191978  -9.55607377143667e-05  6.19576039900837e-06 -2.30466608213628e-07 3.74605718435540e-09
