...
function SAFTVRQMie(components; idealmodel=BasicIdeal, userlocations=String[], ideal_userlocations=String[], reference_state=nothing, fh_order=:fh2, verbose=false)
...
function deff(model::SAFTVRQMieModel, V, T, z, _data)
    ...
    function udud2u(x)
        ...
        Q2λr = @f(Q2, λr)
        Q2λa = @f(Q2, λa)
        _u = _C*((x^-λr-x^-λa) + Di*(Q1λr*x^-(λr+2)-Q1λa*x^-(λa+2)) + Di^2*(Q2λr*x^-(λr+4)-Q2λa*x^-(λa+4)))
        _du = -_C*((λr*x^-(λr+1)-λa*x^-(λa+1)) + Di*(Q1λr*(λr+2)*x^-(λr+3)-Q1λa*(λa+2)*x^-(λa+3)) + Di^2*(Q2λr*(λr+4)*x^-(λr+5)-Q2λa*(λa+4)*x^-(λa+5)))
        _d2u = _C*((λr*(λr+1)*x^-(λr+2)-λa*(λa+1)*x^-(λa+2)) + Di*(Q1λr*(λr+2)*(λr+3)*x^-(λr+4)-Q1λa*(λa+2)*(λa+3)*x^-(λa+4)) + Di^2*(Q2λr*(λr+4)*(λr+5)*x^-(λr+6)-Q2λa*(λa+4)*(λa+5)*x^-(λa+6)))
...
function ϵeff(model::SAFTVRQMieModel, V, T, z)
    ...
    function u(x, λa, λr, σ, T, Mw)
        ...
        _C = @f(Cλ, λa, λr)
        Dij = ħ^2/(12*k_B*T*Mw/N_A*σ^2)
        return _C*((x^-λr-x^-λa) + Dij*(Q1λr*x^-(λr+2)-Q1λa*x^-(λa+2)) + Dij^2*(Q2λr*x^-(λr+4)-Q2λa*x^-(λa+4)))
    end
...
function a_disp(model::SAFTVRQMieModel, V, T, z, _data=@f(data))
    ...
    for i ∈ comps
        ...
        a1_ij = 2 * π * ϵ * dij3 * _C * _ρ_S * ((x_0ij^λa*(@f(aS_1, λa, ζₓ)+@f(B, λa, x_0effij, ζₓ)) - x_0ij^λr*(@f(aS_1, λr, ζₓ)+@f(B, λr, x_0effij, ζₓ))) + (x_0ij^(λa+2)*Q1λa*(@f(aS_1, λa+2, ζₓ)+@f(B, λa+2, x_0effij, ζₓ)) - x_0ij^(λr+2)*Q1λr*(@f(aS_1, λr+2, ζₓ)+@f(B, λr+2, x_0effij, ζₓ)))*Dij + (x_0ij^(λa+4)*Q2λa*(@f(aS_1, λa+4, ζₓ)+@f(B, λa+4, x_0effij, ζₓ)) - x_0ij^(λr+4)*Q2λr*(@f(aS_1, λr+4, ζₓ)+@f(B, λr+4, x_0effij, ζₓ)))*Dij^2)
        #calculations for a2 - diagonal
        σcoeff = σ/σeff
        α = _C*ϵ/ϵff * ((σcoeff^λa/(λa-3)-σcoeff^λr/(λr-3)) + Dij*(σcoeff^(2+λa)*Q1λa/(λa-1) - σcoeff^(2+λr)*Q1λr/(λr-1)) + Dij^2*(σcoeff^(4+λa)*Q2λa/(λa+1) - σcoeff^(4+λr)*Q2λr/(λr+1)))
        ...
        a2_ij =
            ...
            _C^2 *
            (
                x_0ij^(2*λa)*(@f(aS_1, 2*λa, ζₓ)+@f(B, 2*λa, x_0effij, ζₓ)) - x_0ij^(λa+λr)*2*(@f(aS_1, λa+λr, ζₓ)+@f(B, λa+λr, x_0effij, ζₓ)) + x_0ij^(2*λr)*(@f(aS_1, 2*λr, ζₓ)+@f(B, 2*λr, x_0effij, ζₓ)) + x_0ij^(2*λa+2)*2*Q1λa*(@f(aS_1, 2*λa+2, ζₓ)+@f(B, 2*λa+2, x_0effij, ζₓ))*Dij + x_0ij^(2*λr+2)*2*Q1λr*(@f(aS_1, 2*λr+2, ζₓ)+@f(B, 2*λr+2, x_0effij, ζₓ))*Dij - x_0ij^(λa+λr+2)*2*(Q1λa+Q1λr)*(@f(aS_1, λa+λr+2, ζₓ)+@f(B, λa+λr+2, x_0effij, ζₓ))*Dij +
                x_0ij^(2*λa+4)*Q1λa^2*(@f(aS_1, 2*λa+4, ζₓ)+@f(B, 2*λa+4, x_0effij, ζₓ))*Dij^2 +
                x_0ij^(2*λr+4)*Q1λr^2*(@f(aS_1, 2*λr+4, ζₓ)+@f(B, 2*λr+4, x_0effij, ζₓ))*Dij^2 - x_0ij^(λa+λr+4)*(2*Q1λa*Q1λr)*(@f(aS_1, λa+λr+4, ζₓ)+@f(B, λa+λr+4, x_0effij, ζₓ))*Dij^2 +
                x_0ij^(2*λa+4)*2*Q2λa*(@f(aS_1, 2*λa+4, ζₓ)+@f(B, 2*λa+4, x_0effij, ζₓ))*Dij^2 +
                x_0ij^(2*λr+4)*2*Q2λr*(@f(aS_1, 2*λr+4, ζₓ)+@f(B, 2*λr+4, x_0effij, ζₓ))*Dij^2 - x_0ij^(λa+λr+4)*2*(Q2λa+Q2λr)*(@f(aS_1, λa+λr+4, ζₓ)+@f(B, λa+λr+4, x_0effij, ζₓ))*Dij^2 +
                x_0ij^(2*λa+6)*2*(Q1λa*Q2λa)*(@f(aS_1, 2*λa+6, ζₓ)+@f(B, 2*λa+6, x_0effij, ζₓ))*Dij^3 +
                x_0ij^(2*λr+6)*2*(Q1λr*Q2λr)*(@f(aS_1, 2*λr+6, ζₓ)+@f(B, 2*λr+6, x_0effij, ζₓ))*Dij^3 - x_0ij^(λa+λr+6)*2*(Q1λr*Q2λa+Q1λa*Q2λr)*(@f(aS_1, λa+λr+6, ζₓ)+@f(B, λa+λr+6, x_0effij, ζₓ))*Dij^3 +
                x_0ij^(2*λa+8)*Q2λa^2*(@f(aS_1, 2*λa+8, ζₓ)+@f(B, 2*λa+8, x_0effij, ζₓ))*Dij^4 +
                x_0ij^(2*λr+8)*Q2λr^2*(@f(aS_1, 2*λr+8, ζₓ)+@f(B, 2*λr+8, x_0effij, ζₓ))*Dij^4 - x_0ij^(λa+λr+8)*(2*Q2λa*Q2λr)*(@f(aS_1, λa+λr+8, ζₓ)+@f(B, λa+λr+8, x_0effij, ζₓ))*Dij^4
            )
        ...
        for j ∈ (i + 1):l
            ...
            _χ = f1*_ζst+f2*_ζst5+f3*_ζst8
            a2_ij =
                π*_KHS*(1+_χ)*_ρ_S*ϵ^2*dij3*_C^2*(
                    x_0ij^(2*λa)*(@f(aS_1, 2*λa)+@f(B, 2*λa, x_0effij, ζₓ)) - x_0ij^(λa+λr)*2*(@f(aS_1, λa+λr)+@f(B, λa+λr, x_0effij, ζₓ)) + x_0ij^(2*λr)*(@f(aS_1, 2*λr)+@f(B, 2*λr, x_0effij, ζₓ)) + x_0ij^(2*λa+2)*2*Q1λa*(@f(aS_1, 2*λa+2)+@f(B, 2*λa+2, x_0effij, ζₓ))*Dij + x_0ij^(2*λr+2)*2*Q1λr*(@f(aS_1, 2*λr+2)+@f(B, 2*λr+2, x_0effij, ζₓ))*Dij - x_0ij^(λa+λr+2)*2*(Q1λa+Q1λr)*(@f(aS_1, λa+λr+2)+@f(B, λa+λr+2, x_0effij, ζₓ))*Dij +
                    x_0ij^(2*λa+4)*Q1λa^2*(@f(aS_1, 2*λa+4, ζₓ)+@f(B, 2*λa+4, x_0effij, ζₓ))*Dij^2 +
                    x_0ij^(2*λr+4)*Q1λr^2*(@f(aS_1, 2*λr+4, ζₓ)+@f(B, 2*λr+4, x_0effij, ζₓ))*Dij^2 - x_0ij^(λa+λr+4)*(2*Q1λa*Q1λr)*(@f(aS_1, λa+λr+4, ζₓ)+@f(B, λa+λr+4, x_0effij, ζₓ))*Dij^2 +
                    x_0ij^(2*λa+4)*2*Q2λa*(@f(aS_1, 2*λa+4, ζₓ)+@f(B, 2*λa+4, x_0effij, ζₓ))*Dij^2 +
                    x_0ij^(2*λr+4)*2*Q2λr*(@f(aS_1, 2*λr+4, ζₓ)+@f(B, 2*λr+4, x_0effij, ζₓ))*Dij^2 - x_0ij^(λa+λr+4)*2*(Q2λa+Q2λr)*(@f(aS_1, λa+λr+4, ζₓ)+@f(B, λa+λr+4, x_0effij, ζₓ))*Dij^2 +
                    x_0ij^(2*λa+6)*2*(Q1λa*Q2λa)*(@f(aS_1, 2*λa+6)+@f(B, 2*λa+6, x_0effij, ζₓ))*Dij^3 +
                    x_0ij^(2*λr+6)*2*(Q1λr*Q2λr)*(@f(aS_1, 2*λr+6)+@f(B, 2*λr+6, x_0effij, ζₓ))*Dij^3 - x_0ij^(λa+λr+6)*2*(Q1λr*Q2λa+Q1λa*Q2λr)*(@f(aS_1, λa+λr+6)+@f(B, λa+λr+6, x_0effij, ζₓ))*Dij^3 +
                    x_0ij^(2*λa+8)*Q2λa^2*(@f(aS_1, 2*λa+8, ζₓ)+@f(B, 2*λa+8, x_0effij, ζₓ))*Dij^4 +
                    x_0ij^(2*λr+8)*Q2λr^2*(@f(aS_1, 2*λr+8, ζₓ)+@f(B, 2*λr+8, x_0effij, ζₓ))*Dij^4 - x_0ij^(λa+λr+8)*(2*Q2λa*Q2λr)*(@f(aS_1, λa+λr+8, ζₓ)+@f(B, λa+λr+8, x_0effij, ζₓ))*Dij^4
                )
