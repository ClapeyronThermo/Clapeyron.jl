...
function structSAFTgammaMie(components; idealmodel          = BasicIdeal, userlocations       = String[], group_userlocations = String[], ideal_userlocations = String[], reference_state     = nothing, verbose             = false, epsilon_mixing      = :default, assoc_options       = AssocOptions())
...
function a_chain(model::structSAFTgammaMieModel, V, T, z, _data=@f(data))
    ...
    for k ∈ @groups
        ...
        for l in 1:(k - 1)
            ...
            #@show (g_HSi,i)
            ∂a_1∂ρ_S = _C*(x_0^λa*(∂aS_1∂ρS_a+∂B∂ρS_a) - x_0^λr*(∂aS_1∂ρS_r+∂B∂ρS_r))
            #@show (∂a_1∂ρ_S,1)
            ...
            θ = exp(ϵ/T)-1
            γc = 10 * (-tanh(10*(0.57-α))+1) * _ζst * θ * exp(-6.7*_ζst-8*_ζst^2)
            ∂a_2∂ρ_S = 0.5 * _C^2 * (ρS_∂KHS*(x_0^(2*λa)*(aS_1_2a+B_2a) - 2*x_0^(λa+λr)*(aS_1_ar+B_ar) + x_0^(2*λr)*(aS_1_2r+B_2r)) + _KHS*(x_0^(2*λa)*(∂aS_1∂ρS_2a+∂B∂ρS_2a) - 2*x_0^(λa+λr)*(∂aS_1∂ρS_ar+∂B∂ρS_ar) + x_0^(2*λr)*(∂aS_1∂ρS_2r+∂B∂ρS_2r)))

            gMCA2 = 3*∂a_2∂ρ_S-_KHS * _C^2 * (λr*x_0^(2*λr)*(aS_1_2r+B_2r) - (λa+λr)*x_0^(λa+λr)*(aS_1_ar+B_ar) + λa*x_0^(2*λa)*(aS_1_2a+B_2a))
            g_2_ = (1+γc)*gMCA2
            #@show (g_2_,i)
