...
function J2(model, V, T, z, type, i, j, η = @f(ζ,3),m = model.params.segment.values,ϵ = model.params.epsilon.values)
    ...
    consts = _get_ppcsaft_consts(type)
    aij = _ppcsaft_corr_poly(consts.corr_a,m̄,type)
    bij = _ppcsaft_corr_poly(consts.corr_b,m̄,type)
    cij = aij .+ bij .* ϵT
    return evalpoly(η,cij)
end

function J3(model, V, T, z, type, i, j, k, η = @f(ζ,3),m = model.params.segment.values)
    corr_c = _get_ppcsaft_consts(type).corr_c
    m̄ = cbrt(m[i]*m[j]*m[k])
    if type == :DD m̄ = min(m̄,2*one(η)) end
    if type == :DQ
        m̄1 = 1.
    ...
    cijk = _ppcsaft_corr_poly(corr_c,m̄,type)
    return evalpoly(η,cijk)
end

function _get_ppcsaft_consts(type::Symbol)
    if type == :DD return DD_consts
    elseif type == :QQ return QQ_consts
    else return DQ_consts end
end

function _ppcsaft_corr_poly(corr,m̄,type)
    corr_len = type == :DQ ? 4 : 5
    m̄1 = 1.
    m̄2 = (m̄-1)/m̄
    m̄3 = m̄2*(m̄-2)/m̄
    m̄i = (m̄1,m̄2,m̄3)
    return NTuple{corr_len}(dot(m̄i,ci) for ci in corr)
end
