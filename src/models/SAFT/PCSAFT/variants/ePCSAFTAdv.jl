abstract type ePCSAFTAdvModel <: ESElectrolyteModel end

struct ePCSAFTAdv{T<:IdealModel,c<:EoSModel,i<:IonModel} <: ePCSAFTAdvModel
...
function ePCSAFTAdv(solvents,ions; 
    ...
    neutral_path = DB_PATH.*["/SAFT/PCSAFT","/SAFT/PCSAFT/ePCSAFTAdv","/SAFT/PCSAFT/pharmaPCSAFT"]
    ...
    model = ePCSAFTAdv(components,icomponents,charge,init_idealmodel,init_neutralmodel,init_ionmodel,references)
    return model
end

function a_res(model::ePCSAFTAdvModel, V, T, z)
    data_pcsaft = data(model.neutralmodel,V,T,z)
    data_ion = data(model.ionmodel,V,T,z)
    data_ion = (data_ion[1],data_pcsaft[1],data_pcsaft[1])
    # println(data_ion)
