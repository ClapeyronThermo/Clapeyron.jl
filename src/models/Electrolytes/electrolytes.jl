...
"""
function molality_to_composition(model::ElectrolyteModel, salts::Union{AbstractVector,GroupParam}, m, zsolv=SA[1.0], ν=salt_stoichiometry(model, salts))
    nc = length(model)
    Z = model.charge
...
ion_eltype(model, ::Val{true}) = Base.promote_type(paramtype(model), eltype(model.RSPmodel))
ion_eltype(model, ::Val{false}) = paramtype(model)
