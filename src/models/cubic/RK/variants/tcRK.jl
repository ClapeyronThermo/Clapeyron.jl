...
## References
 ...
 3. Piña-Martinez, A., Privat, R., & Jaubert, J.-N. (2022). Use of 300,000 pseudo‐experimental data over 1800 pure fluids to assess the performance of four cubic equations of state: SRK , PR , tc ‐RK , and tc ‐PR. AIChE Journal. American Institute of Chemical Engineers, 68(2). [doi:10.1002/aic.17518](https://doi.org/10.1002/aic.17518)
"""
function tcRK(components; idealmodel=BasicIdeal, alpha=tcTwuAlpha, mixing=vdW1fRule, activity=nothing, translation=tcTranslation, userlocations=String[], ideal_userlocations=String[], alpha_userlocations=String[], mixing_userlocations=String[], activity_userlocations=String[], translation_userlocations=String[], reference_state=nothing, verbose=false)
    ...
    params = getparams(formatted_components, ["cubic/tcRK/tcRK_single.csv"]; userlocations=tc_userlocations, verbose=verbose, ignore_missing_singleparams=["v_shift", "ZRA", "acentricfactor", "M", "N", "L"])
    ...
    model = CubicModel(RK, params, formatted_components; alpha=_alpha, translation=_translation, idealmodel, mixing, activity, userlocations, ideal_userlocations, alpha_userlocations, activity_userlocations, mixing_userlocations, translation_userlocations, reference_state, verbose)
