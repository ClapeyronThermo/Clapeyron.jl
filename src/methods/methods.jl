...
@inline uvstrip(model,v,z) = u_vstrip(unit_system(v),model,v,z)
@inline uvstrip(::Nothing,model,v,z) = v

@inline uzstrip(model,z::Number) = u_vstrip(unit_system(v),model,SA[z])
@inline uzstrip(model,z) = u_vstrip(unit_system(v),model,z)
...
function with_output_unit(::Val{true},res::R,t::Tuple{X,Y},f::F) where {R,F,X,Y}
    unit_sys0,output = t
    unit_sys = unit_system(unit_sys,unit_system(output))
