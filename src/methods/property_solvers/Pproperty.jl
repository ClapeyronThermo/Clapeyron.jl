...
function _Pproperty(model::EoSModel,T,_prop,z = SA[1.0],
    ...
    if length(z) == 1
        ...
        pnc1,ptnc1,_ = Pproperty_pure(fluid_model(model),T,prop,z,property,rootsolver,phase,abstol,reltol,verbose,threaded,p0)
        return __Pproperty_check((pnc1,ptnc1),verbose)
    end

    if p0 !== nothing
        if is_unknown(phase)
            phase_p0 = identify_phase(model,p0,T,z)
        else
            phase_p0 = phase
        end
    ...
    if !(0 <= βpuresat <= 1)  #TODO: check if this is valid
        ...
        if verbose
            ...
            verbose && Xproperty_verbose(:puresat_p, phase_by_puresat, property)
        end
...
function Pproperty_supercritical(model,T,prop,z,property,cache,sol_options,phase)
    ...
    if is_liquid(phase) || is_vapour(phase)
        res_with_phase = __Pproperty(model,T,prop,z,property,rootsolver,phase,abstol,reltol,false,Pc)
        return __Pproperty_check(res_with_phase,verbose)
    end
    if verbose
...
function Pproperty_pure(model,T,x,z,property::F,rootsolver,phase,abstol,reltol,verbose,threaded,p0) where F
    ...
    if !isnothing(p0) && is_unknown(phase)
        phase_p0 = identify_phase(model,p0,T,z)
    else
        phase_p0 = phase
    end
    ...
    if !isnothing(p0)
        ...
        p_init,_ = __Pproperty(model,T,x,z,property,rootsolver,phase_p0,abstol,reltol,false,p0)
        return p_init,phase_p0,nan
    end
...
function __Pproperty(model,T,prop,property::F,rootsolver,phase,abstol,reltol,threaded,p0) where F
    __Pproperty(model,T,prop,SA[1.0],property,rootsolver,phase,abstol,reltol,threaded,p0)
end
#=
