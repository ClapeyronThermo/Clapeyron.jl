function ∂Helmholtz(model::EoSModel, ρ, T, z=[1.0])
    # Auxiliar functions to compute Helmholtz free energy and its first density derivative
...
function ∂2Helmholtz(model::EoSModel, ρ, T, z=[1.0])
    # Auxiliar functions to compute Helmholtz free energy and its first and second density derivatives
    f(dρ) = eos(model, 1. / dρ, T, z)
