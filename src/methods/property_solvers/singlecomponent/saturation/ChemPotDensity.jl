...
function fobj_psat!(model::EoSModel, T)
    ...
    function fj!(F, J, x)
        ...
        ∂P_liq = 2.0 * ρ_liq * ∂A_liq + ρ_liq^2 * ∂2A_liq
        ∂μ_liq = ρ_liq*∂2A_liq + 2*∂A_liq
        A_vap, ∂A_vap, ∂2A_vap = ∂2Helmholtz(model, ρ_vap, T)
        ∂P_vap = 2.0 * ρ_vap * ∂A_vap + ρ_vap^2 * ∂2A_vap
...
"""
function ChemPotDensitySaturation(; vl=nothing, vv=nothing, crit=nothing, f_limit=0.0, atol=1e-8, rtol=1e-12, max_iters=10^4)
...
function psat_chempot(model, T, vol_liq0, vol_vap0, options=NEqOptions())
    ...
    sol = Solvers.nlsolve(ofpsat, ρ0, LineSearch(Newton(), Backtracking()), options)
    #@show sol
    ρ = Solvers.x_sol(sol)
    vol_liq, vol_vap = 1 ./ ρ
