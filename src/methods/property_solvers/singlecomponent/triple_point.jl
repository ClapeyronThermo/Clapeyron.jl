function obj_triple_point(model::CompositeModel,F,T,vs,vl,vv,p_scale,T_scale)
    ...
    As,Avs = Solvers.f∂f(f_solid,vs,∂Tag{:obj_triple_point}())
    Al,Avl = Solvers.f∂f(f_fluid,vl,∂Tag{:obj_triple_point}())
    Av,Avv = Solvers.f∂f(f_fluid,vv,∂Tag{:obj_triple_point}())
