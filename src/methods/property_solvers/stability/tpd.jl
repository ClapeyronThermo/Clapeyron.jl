...
Returns a Tuple, containing:
  ...
  -
"""
function tpd_solver(model, p, T, z, w0, dz          = nothing, cache       = tpd_cache(model, p, T, z, w0); phasew      = :unknown, break_first = false, tol_trivial = 1e-5, tol_equil   = 1e-10, it_ss       = 30, lle         = false)
...
function tpd_optimization(model, p, T, z, w0, di, cache=tpd_cache(model, p, T, z, K0), phasew=:liquid)
    ...
    res = Solvers.optimize(prob, α0, LineSearch(Newton2(α0), Solvers.BoundedLineSearch(lb, ub)), opt_options)
    α = Solvers.x_sol(res)
    w .= α .* α .* 0.25
    S = sum(w)
    w ./= sum(w)
