...
"""
function mean_ionic_activity_coefficient_sat(model::ESElectrolyteModel, salts::Union{AbstractVector,GroupParam}, T::Number, m::Number, zsolv=SA[1.0])
...
If multiple solvents are present, the composition of the solvent can be specified with the `zsolv` keyword argument.
"""
function mean_ionic_activity_coefficient(model::ESElectrolyteModel, salts::Union{AbstractVector,GroupParam}, p, T, m, zsolv=SA[1.0], sat=false)
...
function mean_ionic_activity_coefficient(model::ESElectrolyteModel, p::Number, T, m, zsolv=SA[1.0], sat=false)
