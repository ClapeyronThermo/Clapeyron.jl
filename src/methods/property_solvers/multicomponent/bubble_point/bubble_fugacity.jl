...
function FugBubblePressure(; vol0         = nothing, p0           = nothing, y0           = nothing, nonvolatiles = nothing, f_limit      = 0.0, atol         = 1e-8, rtol         = 1e-12, max_iters    = 10^4, itmax_newton = 10, itmax_ss     = 5, tol_y        = 1e-8, tol_p        = 1e-8, tol_of       = 1e-8, second_order = true)
...
function bubble_pressure_impl(model::EoSModel, T, x, method::FugBubblePressure)
    ...
    data = FugData(FugEnum.BUBBLE_PRESSURE, method.itmax_ss, method.itmax_newton, method.tol_p, method.tol_y, method.tol_of, method.second_order, false)
    ...
    if all(volatiles)
        ...
        sol = Solvers.nlsolve(problem, inc0, LineSearch(Newton2(inc0), Backtracking()), opts)
...
function FugBubbleTemperature(; vol0         = nothing, T0           = nothing, y0           = nothing, nonvolatiles = nothing, f_limit      = 0.0, atol         = 1e-8, rtol         = 1e-12, max_iters    = 10^4, itmax_newton = 10, itmax_ss     = 5, tol_y        = 1e-8, tol_T        = 1e-8, tol_of       = 1e-8, second_order = true)
...
function bubble_temperature_impl(model::EoSModel, p, x, method::FugBubbleTemperature)
    ...
    data = FugData(FugEnum.BUBBLE_TEMPERATURE, method.itmax_ss, method.itmax_newton, method.tol_T, method.tol_y, method.tol_of, method.second_order, false)
