function solid_solubility(model::CompositeModel,p,T,z;solute=nothing)
    if isnothing(solute)
    ...
    for i in 1:length(solute)
        ...
        x0 = x0_solid_solubility(model,p,T,z,idx_sol,μsol)
        # println(x0)
        f!(F,x) = obj_solid_solubility(F,model.liquid,p,T,z[.!(idx_sol)],exp10(x[1]),μsol,idx_sol)
...
function x0_solid_solubility(model,p,T,z,idx_sol,μsol)
    z∞ = zeros(length(z))
    z∞[.!(idx_sol)] = z[.!(idx_sol)]
    z∞[idx_sol] .= 1e-10
    z∞ ./= sum(z∞)
