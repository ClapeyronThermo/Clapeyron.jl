...
"""
function MultiPhaseTPFlash(; K0           = nothing, x0           = nothing, y0           = nothing, n0           = nothing, K_tol        = sqrt(eps(Float64)), ss_iters     = 6, nacc         = 5, second_order = true, full_tpd     = false, max_phases   = typemax(Int), flash_result = nothing, phase_iters  = 20, verbose      = false) #TODO: find a better value for this
...
function RR_obj(x, z, _result, ss_cache, deactivated_phase)
    ...
    return ScalarObjective(f=f, g=g, fg=fg, fgh=fgh, h=h)
end
