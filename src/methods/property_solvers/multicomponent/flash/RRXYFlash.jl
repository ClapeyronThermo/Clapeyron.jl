...
function Solvers.primalval(method::RRXYFlash{P,T}) where {P,T}
    ...
    return RRXYFlash{λP,λT}(method.equilibrium, primalval(method.T0), primalval(method.p0), primalval(method.K0), primalval(method.x0), primalval(method.y0), primalval(method.v0), method.tol_xy, method.tol_pT, method.tol_of, method.max_iters, method.ss_iters, method.verbose)
end
...
function RRXYFlash(; equilibrium=:unknown, T0=nothing, p0=nothing, K0=nothing, x0=nothing, y0=nothing, v0=nothing, tol_xy=1e-10, tol_pT=1e-12, tol_of=1e-10, max_iters=100, ss_iters=5, flash_result=nothing, verbose=false)
    ...
    if flash_result isa FlashResult
        ...
        return RRXYFlash(; equilibrium=equilibrium, T0=T00, p0=P00, x0=w1, y0=w2, v0=v, tol_xy=tol_xy, tol_pT=tol_pT, tol_of=tol_of, max_iters=max_iters, ss_iters=ss_iters, verbose=verbose)
    end
    ...
    return RRXYFlash{S,TT}(equilibrium, T0, p0, K0, x0, y0, _v0, tol_xy, tol_pT, tol_of, max_iters, ss_iters, verbose)
end
