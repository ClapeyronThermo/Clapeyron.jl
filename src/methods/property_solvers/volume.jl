...
function _volume_compress_bisection(model,p,T,z,data1,data2,i0,max_iters)
    ...
    # order the bracket by pressure: plo < p < phi.
    # on a mechanically stable branch dp/dV < 0, so vhi < vlo.
    if p1 > p2
        vhi,logvhi,phi,dpdvhi = v1,logv1,p1,dpdv1
        vlo,logvlo,plo,dpdvlo = v2,logv2,p2,dpdv2
    else
        vlo,logvlo,plo,dpdvlo = v1,logv1,p1,dpdv1
        vhi,logvhi,phi,dpdvhi = v2,logv2,p2,dpdv2
    end

    # initial current iterate = endpoint closer to the target pressure
    if abs(p1 - p) <= abs(p2 - p)
        logvᵢ,Vᵢ,pᵢ,dpdVᵢ = logv1,v1,p1,dpdv1
    else
        logvᵢ,Vᵢ,pᵢ,dpdVᵢ = logv2,v2,p2,dpdv2
    end

    # best iterate seen so far, in the sense of smallest |p_new - p|
    best_V    = Vᵢ
    best_err  = abs(pᵢ - p)
    _logVᵢ = logvᵢ
    ...
    for i in i0:max_iters
        # near-critical fast path
        abs(pᵢ - p) < 3eps(p) && return Vᵢ

        _logVᵢ = logvᵢ

        # 1) unguarded full Newton step from the current iterate
        Δᵢ = (p - pᵢ)/(Vᵢ*dpdVᵢ)
        logv_new = logvᵢ + Δᵢ

        # 2) if the Newton step leaves the bracket, use mean newton bracketed update instead
        if !(logvhi < logv_new < logvlo)
            Δlo = (p - plo)/(vlo*dpdvlo)
            Δhi = (p - phi)/(vhi*dpdvhi)
            iszero(Δhi) && return vhi
            iszero(Δlo) && return vlo
            δlo = clamp((logvhi - logvlo)/Δlo,_0,one(_0))
            δhi = clamp((logvlo - logvhi)/Δhi,_0,one(_0))
        ...
        # 3) update bounds from the new evaluation
        if (p_new - p)*(plo - p) < _0
        ...
        # 5) same convergence criterion as _volume_compress

        converged && break
        logvᵢ,Vᵢ,pᵢ,dpdVᵢ = logv_new,v_new,p_new,dpdv_new
        converged,finite = Solvers.convergence(_logVᵢ,logv_new,zero(logv_new),1e-12)
    end
    return finite ? best_V : nan
end
