...
function x0_sat_pure(model::SAFTModel)
    x0    = [log10(π/6*N_A*model.params.segment.values[1]*model.params.sigma.values[1]^3/0.5),
    log10(π/6*N_A*model.params.segment.values[1]*model.params.sigma.values[1]^3/1e-3)]
end
...
function scale_sat_pure(model::SAFTModel)
    p_scale    = model.params.sigma.values[1]^3*N_A/R̄/model.params.epsilon.values[1]
    μ_scale    = 1/R̄/model.params.epsilon.values[1]
    return p_scale,μ_scale
end
