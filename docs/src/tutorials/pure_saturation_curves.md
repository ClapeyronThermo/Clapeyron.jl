...
Ideally, once the saturation pressure and temperature are determined, one should be able to input those values into the bulk property calculators, specifying the respective phases.
...
Rather than taking the pressure as an input, these functions take the system's volume.
