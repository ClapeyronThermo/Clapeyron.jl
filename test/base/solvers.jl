...
@testset verbose = true "Solvers Module" begin
    ...
    @testset "nlsolve" begin
        res = SOL.nlsolve(f_diffmcp!,[0.0],TrustRegion(SOL.Newton()),SOL.NLSolvers.NEqOptions(),ForwardDiff.Chunk{1}())
        res2 = SOL.nlsolve(f_diffmcp!,[0.0],LineSearch(SOL.Newton(),SOL.Backtracking()),SOL.NLSolvers.NEqOptions(),ForwardDiff.Chunk{1}())
