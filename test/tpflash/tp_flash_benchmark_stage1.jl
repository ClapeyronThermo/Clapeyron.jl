...
function tpflash_benchmark_stage1(;
    ...
    if verbose
        ...
        println("- failure_tags:    NUMERIC_FAIL, CONSTRAINT_FAIL, TARGET_FAIL, X_FAIL, BETA_FAIL")
    end
    ...
    for (ci, case) in enumerate(cases)
        if verbose
            println()
            println("[$ci/$(length(cases))] case: ", case.id)
        end
...
tpflash_benchmark_stage1(algo="rdex", backend=:rdex, seeds=1:10, population_size=50, fe_factor=10_000, time_limit=30.0, stagnation_evals=0, stagnation_tol=0.0, filter=nothing, atol_tgt=1e-12, rtol_tgt=1e-12)
tpflash_benchmark_stage1(algo="bbo", backend=:bbo, seeds=1:10, population_size=50, fe_factor=10_000, time_limit=30.0, stagnation_evals=0, stagnation_tol=0.0, filter=nothing, atol_tgt=1e-12, rtol_tgt=1e-12)

Stage2：
include("test/tpflash/tp_flash_benchmark_stage2.jl");
tpflash_benchmark_stage2(indir="test/tpflash/results")
