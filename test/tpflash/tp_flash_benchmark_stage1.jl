...
function tpflash_benchmark_stage1(;
    ...
    cfg = TPFlashBenchmarkConfig(; backend=backend, seeds=seed_list, population_size=population_size, fe_factor=fe_factor, time_limit=time_limit, stagnation_evals=stagnation_evals, stagnation_tol=stagnation_tol, atol_tgt=atol_tgt, rtol_tgt=rtol_tgt, tau_x=tau_x, tau_beta=tau_beta, beta_min=beta_min, max_e_x=max_e_x, max_e_beta=max_e_beta)
        ...
        for (si, seed) in enumerate(seed_list)
            ...
            if verbose
                ...
                elseif show_fail_details
                    @printf("%s Δ=%0.3e (g=%0.16g tgt=%0.16g) e_x=%0.3e e_beta=%0.3e nfes=%d\n", _format_failure_tags(run.failure_tags), run.Δ_final, run.g_best, run.tgt, run.e_x, run.e_beta, run.nfes,)
                else
...
function main()
    ...
    tpflash_benchmark_stage1(; algo=algo, backend=backend, outdir=outdir, seeds=seeds, population_size=pop, fe_factor=fe_factor, time_limit=time_limit, stagnation_evals=stagnation_evals, stagnation_tol=stagnation_tol, atol_tgt=atol_tgt, rtol_tgt=rtol_tgt, tau_x=tau_x, tau_beta=tau_beta, beta_min=beta_min, max_e_x=max_e_x, max_e_beta=max_e_beta, filter=filter, verbose=true)
    return nothing
end
