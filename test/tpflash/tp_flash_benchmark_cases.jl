...
function tpflash_benchmark_cases()
    ...
    push!(cases, TPFlashBenchmarkCase("2. vle_pr_isobutane_nbutane_npenthane_nhexane_282p2K_1bar", "PR VLE (IsoButane/n-Butane/n-Pentane/n-Hexane), T=282.2K, p=1e5 Pa, z=[0.25,0.25,0.25,0.25]", () -> PR(["IsoButane", "n-Butane", "n-Pentane", "n-Hexane"]), 1e5, 282.2, [0.25, 0.25, 0.25, 0.25], :vle, 2, false, -6.679466011839553, [0.25 0.25 0.25 0.25; 0.25 0.25 0.25 0.25], [1.0, 0.0]))
    ...
    # Reference has x₁≈x₂≈z and β₁≈0, so g can be near-optimal even when β drifts.
    push!(cases, TPFlashBenchmarkCase("4. vle_pr_nbutane_npenthane_nhexane_nheptane_450K_1bar", "PR VLE (n-butane/n-pentane/n-hexane/n-heptane), T=450K, p=1e5 Pa, z=[0.25,0.25,0.25,0.25]", () -> PR(["n-butane", "n-pentane", "n-hexane", "n-heptane"]), 1e5, 450.0, [0.25, 0.25, 0.25, 0.25], :vle, 2, false, -7.279957068200142, [0.25 0.25 0.25 0.25; 0.25 0.25 0.25 0.25], [0.0, 1.0]))
    ...
    # --- LLE (2 phases) ---
    push!(cases, TPFlashBenchmarkCase("6. lle_pcsaft_water_cyclohexane_propane_298K_1bar", "PCSAFT LLE (water/cyclohexane/propane), T=298.15K, p=1e5 Pa, z=[0.5,0.5,0.0]", () -> PCSAFT(["water", "cyclohexane", "propane"]), 1e5, 298.15, [0.5, 0.5, 0.0], :lle, 2, false, -7.577270350886795, [0.9996139542909095 0.00038604570909056877 0.0; 0.00970027330568687 0.9902997266943131 0.0], [0.49529543445276647, 0.5047045655472335]))

    # --- Activity models (LLE + VLE) ---
    push!(cases, TPFlashBenchmarkCase("7. lle_unifac_water_hexane_303p15K_1atm", "UNIFAC LLE (water/hexane), T=303.15K, p=101325 Pa, z=[0.5,0.5]", () -> UNIFAC(["water", "hexane"]), 101325.0, 303.15, [0.5, 0.5], :lle, 2, false, -7.254813021496728, [0.999839188463 0.00016081153699992588; 0.009359875304170373 0.9906401246958296], [0.49535625648867265, 0.5046437435113273]))
    ...
    function model_454()
        return PR(["n-butane", "n-pentane", "n-hexane", "n-heptane"]; idealmodel=AlyLeeIdeal, userlocations=(; Tc=[425.12, 469.7, 507.6, 540.2], Pc=[37.96e5, 33.7e5, 30.25e5, 27.4e5], Mw=[58.1234, 72.15028, 86.17716, 100.20404], acentricfactor=[0.200164, 0.251506, 0.301261, 0.349469], k=[
    ...
    push!(cases, TPFlashBenchmarkCase("10. vle_454_case1_rr", "#454 case 1 (RR): p=153823 Pa, T=321.967K, n=[0.007682,0.9923,1.517e-17,1.918e-31]", model_454, 153_823.0, 321.9670623578307, [0.007682, 0.9923, 1.517e-17, 1.918e-31], :vle, 2, false, 123.7459904164123, [0.00236666244831282 0.9976333375516873 0.0 0.0; 0.007929376297815581 0.9920706237021845 0.0 0.0], [0.044445575669629145, 0.9555544243303709]))

    # NOTE: near bubble/dew boundary: β₂ is small (≈1.49e-3), so the objective can be weakly sensitive to β.
    push!(cases, TPFlashBenchmarkCase("11. vle_454_case2_rr", "#454 case 2 (RR): p=701739.83 Pa, T=430.74K, n=[2.984e-14,0.0615,3.48,2.059]", model_454, 701_739.83, 430.74, [2.984e-14, 0.0615, 3.48, 2.059], :vle, 2, false, 103.70091207385789, [5.306960867832837e-15 0.010962897986764673 0.6212133688150436 0.36782373319818634; 1.9503204646204015e-14 0.023231078615878246 0.7284990198696496 0.24826990151445272], [0.9985112373218122, 0.0014887626781877376]))
    ...
    push!(
        ...
        TPFlashBenchmarkCase(
            ...
            1.985550610608908e6,
            416.6628781711617,
            [55.461373286206445, 0.09264900343401582, 7.265116936961075e-9, 8.855321114218425e-14],
            :vle,
            2,
            false,
            87.73279400794794,
    ...
    push!(cases, TPFlashBenchmarkCase("13. vle_454_case4_rr", "#454 case 4 (RR): p=5.35202e5 Pa, T=393.265K, n=[36.4950,0.005799,1.94e-10,2.0e-15]", model_454, 5.35202e5, 393.265, [36.495044786426966, 0.005798955283355085, 1.9416516061189107e-10, 2.0015179988524742e-15], :vle, 2, false, 92.16253721056026, [0.9998411281799807 0.00015887181469993373 5.319470475390174e-12 0.0; 0.9998411281799807 0.00015887181469993373 5.319470475390174e-12 0.0], [0.0, 1.0]))

    # NOTE: near single-phase degeneracy (see `test/test_methods_api_flash.jl` #454 block).
    push!(cases, TPFlashBenchmarkCase("14. vle_454_case5_rr", "#454 case 5 (RR): p=442595.319 Pa, T=318.920K, n=[18.6979,9.209e-8,2.317e-22,1.932e-32]", model_454, 442_595.31887270656, 318.91991913774194, [18.697907101753938, 9.208988950434023e-8, 2.317361697667793e-22, 1.9317538045050555e-32], :vle, 2, false, 114.68417889588606, [0.9999999950748557 4.9251442179935655e-9 0.0 0.0; 0.9999999950748557 4.9251442179935655e-9 0.0 0.0], [1.0, 0.0]))
    ...
    push!(
        ...
        TPFlashBenchmarkCase(
            ...
            model_454,
            2.2099578494144413e6,
            464.63699168781847,
            [2.7561794126981888e-6, 55.964211412167195, 12.860133735598001, 1.0819681996211576],
            :vle,
            2,
            false,
            84.77315106035215,
    ...
    # NOTE: near single-phase degeneracy (see `test/test_methods_api_flash.jl` #454 block).
    push!(cases, TPFlashBenchmarkCase("16. vle_454_case7_michelsen", "#454 case 7 (Michelsen): p=505777.32 Pa, T=323.960K, n=[75.8306,6.149e-10,0,0]", model_454, 505_777.32016068726, 323.9598978773458, [75.83064821431964, 6.148759359393775e-10, 0.0, 0.0], :vle, 2, false, 112.95608449327719, [0.9999999999918915 8.108541208781074e-12 0.0 0.0; 0.9999999999918915 8.108541208781074e-12 0.0 0.0], [1.0, 0.0]))
