...
@testset verbose = true "Ideal models" begin
    ...
    let T = 298.15, V = 1e-4, p = 1e5, z = Clapeyron.SA[1.0], z1 = Clapeyron.SA[1.0], z2 = [0.5, 0.5], z3 = [0.333, 0.333, 0.333]
        ...
        @testset "Empiric" begin
            ...
            @test molar_density(model558, 9.259e3, 220.0; phase=:liquid) ≈ 13029.070044557742 rtol = 1e-6
...
@testset verbose = true "Correlations" begin
    ...
    @testset verbose = true "LiquidVolume" begin
        @testset "COSTALD" begin
            ...
            @test volume(system2, 1e5, 300.15, [1.0, 0.0]) ≈ 1.8553472145724288e-5 rtol = 1e-6
        end
        ...
        @testset "DIPPR105Liquid" begin
            ...
            @test volume(system2, 1e5, 300.15, [1.0, 0.0]) ≈ 1.8057164858551208e-5 rtol = 1e-6
        end
        ...
        @testset "RackettLiquid" begin
            ...
            @test volume(system2, 1e5, 300.15, [1.0, 0.0]) ≈ 1.6837207241594103e-5 rtol = 1e-6
        end
        ...
        @testset "YamadaGunnLiquid" begin
            ...
            @test volume(system2, 1e5, 300.15, [1.0, 0.0]) ≈ 2.0757546189420953e-5 rtol = 1e-6
        end
    ...
    @testset verbose = true "Solid Models" begin
        @testset "SolidHfus" begin
            ...
            @test chemical_potential(model, 1e5, 298.15, [1.0])[1] ≈ 549.1488193300384 rtol = 1e-6
        end
