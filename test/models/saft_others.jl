...
@testset verbose = true "CPA" begin
    ...
    let T = 298.15, V = 1e-4, z1 = Clapeyron.SA[1.0], z = [0.5, 0.5], z3 = [0.333, 0.333, 0.333]
        @testset "CPA" begin
            ...
            model1 = CPA(["C3", "C4"]; userlocations=(; Mw=[44.097, 58.124], Tc=[369.83, 425.18], a=[9.11875, 13.14274], b=[0.057834, 0.072081], c1=[0.6307, 0.70771], epsilon_assoc=nothing, bondvol=nothing))
            @test model1 isa CPA
            GC.gc()
...
@testset "COFFEE" begin
    ...
    let T = 298.15, V = 1e-4, z = Clapeyron.SA[1.0]
        @testset "COFFEE" begin
            system = COFFEE(["a1"]; userlocations=(; Mw       = [1.0], segment  = [1.0], sigma    = [3.0;;], epsilon  = [300.0;;], lambda_r = [12;;], lambda_a = [6;;], shift    = [3*0.15], dipole   = [2.0*1.0575091914494172]))
