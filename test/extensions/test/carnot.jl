...
@testset verbose = true "Carnot.jl" begin
    fluids_test = ["dimethyl ether", "isobutane", "Butene", "Neopentane", "isopentane", "pentane", "hexane", "acetone", "cyclopentane", "heptane", "cyclohexane", "benzene", "propane"]

    solverparams_ad = ThermoCycleParameters(autodiff=true, xtol=1e-8, ftol=1e-8, restart_TOL=1e-4, max_iters=1000, N=30, x0_init=:default)

    solverparams_fd = ThermoCycleParameters(autodiff=false, xtol=1e-8, ftol=1e-8, restart_TOL=1e-4, max_iters=1000, N=30, x0_init=:default)
    ...
    @testset "Isentropic Compression - Single Component" begin
        for fluid in fluids_test
            ...
            z1 = [1.0]
            p1 = 101325
            T1 = dew_temperature(fluid_model, p1, z1)[1] + 50
            p2 = 101325*2
    ...
    @testset "Isentropic pump - Single Component" begin
        for fluid in fluids_test
            ...
            z1 = [1.0]
            p1 = 101325
            T1 = bubble_temperature(fluid_model, p1, z1)[1] - 20
            p2 = 101325*2
            h1 = enthalpy(fluid_model, p1, T1, z1)
            s1 = Clapeyron.entropy(fluid_model, p1, T1, z1)
    ...
    @testset "Isentropic Expansion - Single Component" begin
        for fluid in fluids_test
            ...
            z1 = [1.0]
            p1 = 101325*5
            T1 = saturation_temperature(fluid_model, p1)[1] + 100.0
            p2 = 101325
            h1 = enthalpy(fluid_model, p1, T1, z1)
            s1 = Clapeyron.entropy(fluid_model, p1, T1, z1)
    ...
    @testset "issue : #20" begin
        ...
        z = [0.1, 9.9]
        orc = ORC(fluid=fluid, z=z, T_evap_in=383.15, T_evap_out=363.15, T_cond_in=290.0, T_cond_out=310.0, η_pump=0.75, η_expander=0.75, pp_evap=5.0, pp_cond=5.0, ΔT_sc=16.174645986149887, ΔT_sh=6.851340038880649)
