...
@testset "Reported errors" begin
    ...
    @testset "#188" begin
        ...
        =#
        data = (species=["A", "B"], Tc=[18.0, 3.5], Pc=[2.3, 0.1], Mw=[1.0, 1.0], acentricfactor=[0.1, 0.3])

        file = ParamTable(:single, data, name="db1")
    ...
    @testset "#75 - regression" begin
        ...
        userlocations = (Mw=[352.77, 18.01], segment=[14.174, 1.2046817736], sigma=[3.372, 2.7927], epsilon=[221.261, 353.9449], n_H=[2, 1], n_e=[2, 1], k=[0 -0.155; -0.155 0], kT=[0 2.641e-4; 2.641e-4 0], epsilon_assoc=Dict((("griseofulvin", "e"), ("griseofulvin", "H")) => 1985.49, (("water08", "e"), ("water08", "H")) => 2425.6714), bondvol=Dict((("griseofulvin", "e"), ("griseofulvin", "H")) => 0.02, (("water08", "e"), ("water08", "H")) => 0.04509))
    ...
    @testset "SorptionModels.jl - init kij with user" begin
        ...
        model1 = Clapeyron.SL(["PC", "CO2"], userlocations=Dict(:vol => v★.(P★, T★), :segment => r.(P★, T★, ρ★, mw), :epsilon => ϵ★.(T★), :Mw => mw), mixing_userlocations=(; k0=kij, k1=[0 0; 0 0], l=[0 0; 0 0]))

        model2 = Clapeyron.SL(["PC", "CO2"], userlocations=Dict(:vol => v★.(P★, T★), :segment => r.(P★, T★, ρ★, mw), :epsilon => ϵ★.(T★), :Mw => mw, :k => kij))
    ...
    @testset "#357 - electrolyte equilibria" begin
        ...
        bubT1_2 = bubble_temperature(model1, 101325, x1, FugBubbleTemperature(nonvolatiles=["calcium", "chloride"], y0=[1.0, 0.0, 0.0], vol0=(1.8e-5, 1.0), T0=373.15))[1]
        @test bubT1_2 ≈ bub_test rtol = 1e-6

        bubP1 = bubble_pressure(model1, 298.15, x1, FugBubblePressure(nonvolatiles=["calcium", "chloride"], y0=[1.0, 0.0, 0.0], vol0=(1e-5, 1.0)))[1]
    ...
    @testset "#416" begin
        ...
        model = PR(["nitrogen"]; userlocations=(; Tc = [0.3], Pc = [1.0], Mw = [1.0]))
